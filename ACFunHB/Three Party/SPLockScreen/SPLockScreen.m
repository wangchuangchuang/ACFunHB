...
- (void)handlePanAtPoint:(CGPoint)point
{
    ...
    // wy add 去掉重复点的操作
    NSString *strPattern = [NSString stringWithFormat:@"%@", [self patternToUniqueId]];
    NSString *strCurrentCellIndex = [NSString stringWithFormat:@"%ld", cellPos + 1];
    
    if (![strPattern isEqualToString:@"0"] && (strPattern.length > 1))
    {
        NSRange range = [strPattern rangeOfString:strCurrentCellIndex];
